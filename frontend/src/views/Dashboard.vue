        ...
        <ul class="space-y-4">
          <li v-for="notif in notifications" :key="notif.id" class="bg-gray-700 p-5 rounded-lg border border-gray-600 flex flex-col sm:flex-row justify-between sm:items-center group">
            <div class="mb-4 sm:mb-0">
              ...
              <div v-if="notif.schedule_type === 'webhook'" class="text-xs text-indigo-300 mt-1 flex items-center space-x-2">
                <span>Webhook URL: </span>
                <code class="bg-gray-800 px-2 py-0.5 rounded text-pink-300 select-all max-w-[200px] truncate" :title="getWebhookUrl(notif.schedule_expr)">{{ getWebhookUrl(notif.schedule_expr) }}</code>
                <button @click="copyToClipboard(getWebhookUrl(notif.schedule_expr))" class="text-gray-400 hover:text-white" title="Copy URL">
                  <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>
                </button>
              </div>
              <div v-else class="text-xs text-indigo-300">
      ...
      <div class="bg-gray-800 p-8 pt-6 pb-6 rounded-2xl max-w-5xl w-full border border-gray-600 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <div class="flex-1 flex flex-col md:flex-row gap-8 overflow-hidden min-h-0">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div v-if="notificationForm.schedule_type !== 'webhook'">
          ...
          <div v-if="notificationForm.schedule_type !== 'specific_time' && notificationForm.schedule_type !== 'webhook'" class="mt-4">
          ...
          <div v-if="notificationForm.schedule_type !== 'webhook'" class="space-y-4 mt-6">
...
const saveNotification = async () => {
  try {
    ...
    } else if (payload.schedule_type === 'webhook') {
